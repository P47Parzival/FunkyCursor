...
function applyCursor(cursorFile) {
  if (cursorFile === 'default') {
    document.body.style.cursor = 'auto'; // Reset to default system cursor
...
// Listen for messages (when user selects or resets cursor)
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.cursor) {
    chrome.storage.local.set({ selectedCursor: request.cursor });
